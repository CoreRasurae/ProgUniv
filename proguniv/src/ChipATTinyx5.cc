...
namespace ATTinyx5 {
	...
	//TODO Add constraint checks  -> DWEN should be disabled when memory is locked 
	//	(for example in section 20.1 page 151 of the reference manual) 
    ...
    std::string reverseUpdateInternalClockSource(InterfaceClass::ChipIF& chip) {
    	...
    	//0001 - High frequency (16MHz) PLL Clock
    	//0010 - 8MHz calibrated internal clock source
    	//0011 - 6.4MHz calibrated internal clock source
    	//0100 - 128KHz internal clock source
    	std::string result = reverseUpdateFuseLowCKSEL(chip); 
    	if ( result.compare("0010") == 0 ||
    		 result.compare("0011") == 0 ||
    		 result.compare("0001") == 0 )
    		return "true";
    	else
    		return "false";
    }
...
ChipATTinyx5::ChipATTinyx5() {
	using namespace ATTinyx5;
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "minVddTwd_flash", "4700", "The amount of time required to program the flash memory at minimun Vdd voltage", true ));
	progPropPtr->addAcceptedValue( "4700", "4.7 ms" );
	this->addProgrammerProperty( progPropPtr );
	...
	//Fuse Extended Byte
	propPtr = shared_ptr<Property>(new Property( this, "FUSEEXT_SELFPRGEN", "Disabled", "Self-programming enabled", false ));
	propPtr->addAcceptedValue( "Enabled", "Self-programming is permited" );
	propPtr->addAcceptedValue( "Disabled", "Self-programming is not permited" );
	propPtr->registerForwardUpdater( &updateFuseExtSELFPRGEN );
	...
	propPtr->addAcceptedValue( "0001", "High frequency Internal PLL Oscillator 16 Mhz" );
	propPtr->addAcceptedValue( "0010", "Calibrated Internal RC Oscillator 8.0 MHz" );
	propPtr->addAcceptedValue( "0011", "Calibrated Internal RC Oscillator 6.4 MHz" );
	propPtr->addAcceptedValue( "0100", "Internal 128 kHz RC Oscillator" );
	propPtr->addAcceptedValue( "0101", "Reserved" );
	propPtr->addAcceptedValue( "0110", "Low Frequency Crystal Oscillator (32,768KHz)" );
	propPtr->addAcceptedValue( "0111", "Reserved" );
	propPtr->addAcceptedValue( "1000", "Ceramic Resonator (0.4 - 0.9 MHz) Cfg.0" );
	propPtr->addAcceptedValue( "1001", "Crystal/Ceramic - depends on SUT (0.4 - 0.9 MHz)  Cfg.1" );
	propPtr->addAcceptedValue( "1010", "Ceramic  Resonator (0.9 - 3.0 MHz) Cfg.0" );
	propPtr->addAcceptedValue( "1011", "Crystal/Ceramic - depends on SUT (0.9 - 3.0 MHz)  Cfg.1" );
	propPtr->addAcceptedValue( "1100", "Ceramic  Resonator (3.0 - 8.0 MHz)  Cfg.0" );
	propPtr->addAcceptedValue( "1101", "Crystal/Ceramic - depends on SUT (3.0 - 8.0 MHz)  Cfg.1" );
	propPtr->addAcceptedValue( "1110", "Ceramic Resonator (8.0 - Max. MHz) Cfg.0" );
	propPtr->addAcceptedValue( "1111", "Crystal/Ceramic - depends on SUT (8.0 - Max. MHz) Cfg.1" );
...
bool ChipATTinyx5::compareWithFile(const char* filename) {
	return false;
}

bool ChipATTinyx5::compareWithChip(ChipIF& otherChip) {
	return false;
}
