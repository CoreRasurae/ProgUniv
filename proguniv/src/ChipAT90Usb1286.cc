...
namespace nsChipAT90Usb1286 {
	...
	//Handle ATMega32U4 Fuses for Lock Bit Byte fuses
	...
	//Handle ATMega32U4 Fuse Extended Byte
	void updateFuseExtHwBE(const std::string &value, InterfaceClass::ChipIF& chip) {
		Memoria& mem = *chip.getMemory("Fuse Extended Byte");
		unsigned int old_value;
		mem.le(&old_value);

		old_value |= ~(0x0f); //Set unused bits
	...
	std::string reverseUpdateFuseExtHwBE(InterfaceClass::ChipIF& chip) {
		unsigned int value;
		Memoria& mem = *chip.getMemory("Fuse Extended Byte");
	...
	void updateFuseExtBodLevel(const std::string &value, InterfaceClass::ChipIF& chip) {
		...
		else if ( value.compare("2V6") == 0 ) {
			//bits 2,1,0 = 011
			mem.escreve( old_value | 0x03 );
		}
		else if ( value.compare("3V4") == 0 ) {
			//bits 2,1,0 = 010
			mem.escreve( old_value | 0x02 );
		}
		else if ( value.compare("3V5") == 0 ) {
			//bits 2,1,0 = 001
			mem.escreve( old_value | 0x01 );
		}
	...
	//Handle ATMega32U4 Fuse High Byte Bits
	...
	//Handle ATMega32U4 Fuse Low Byte Bits
    ...
    std::string reverseUpdateInternalClockSource(InterfaceClass::ChipIF& chip) {
    	//This chip has one internal clock source of aprox. 8MHz
...
ChipAT90Usb1286::ChipAT90Usb1286(const std::string &deviceSpecification) {
	using namespace nsChipAT90Usb1286;
	...
	memPtr =  shared_ptr<Memoria>(new Memoria(65536, 0x0000, 16, "Program"));
	this->addMemory( memPtr->obtem_nome(), memPtr );
	memPtr->fill_value(0xffff, 0x0000, 65536);

	memPtr = shared_ptr<Memoria>(new Memoria(4096, 0x0000, 8, "Data"));
	this->addMemory( memPtr->obtem_nome(), memPtr );
	memPtr->fill_value(0xff, 0x0000, 4096);
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "minVddTwd_flash", "6500", "The amount of time required to program the flash memory at minimum Vdd voltage", true ));
	progPropPtr->addAcceptedValue( "6500", "6.5 ms" );
	this->addProgrammerProperty( progPropPtr );

	progPropPtr = shared_ptr<Property>(new Property( this, "minVddTwd_eeprom", "9500", "The amount of time required to program the eeprom memory at minimum Vdd voltage", true ));
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "serialClockCyclesSCK0", "16", "Clock cycles to generate per bit when SCK is 0", true ));
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "serialClockCyclesReadSCK1", "16", "Clock cycles to generate per bit when SCK is 1 on a Read", true ));
	progPropPtr->addAcceptedValue( "16", "16 cycles" );
	this->addProgrammerProperty( progPropPtr );
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "ProgramTotalPages", "512", "The total number of program memory pages in this chip", true ));
	progPropPtr->addAcceptedValue( "512", "The total number of program memory pages is 512" );
	this->addProgrammerProperty( progPropPtr );
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "DataTotalPages", "512", "The total number of program memory pages in this chip", true ));
	progPropPtr->addAcceptedValue( "512", "The total number of data memory pages is 512" );
	this->addProgrammerProperty( progPropPtr );
	...
	//Fuse Extended Byte properties
	propPtr = shared_ptr<Property>(new Property( this, "FUSEEXT_HWBE", "Enabled", "Hardware Boot Enable", false));
	propPtr->addAcceptedValue( "Enabled", "Hardware Boot enabled." );
	propPtr->addAcceptedValue( "Disabled", "Hardware Boot disabled." );
	propPtr->registerForwardUpdater( &updateFuseExtHwBE );
	propPtr->registerReverseUpdater( &reverseUpdateFuseExtHwBE );
	this->addProperty( propPtr );
	extended_memPtr->addMemoryListener( propPtr );

	propPtr = shared_ptr<Property>(new Property( this, "FUSEEXT_BODLEVEL", "Disabled", "Brown-Out Detector trigger level fuses", false ));
	propPtr->addAcceptedValue( "Disabled", "BOD Disabled" );
	propPtr->addAcceptedValue( "Reserved1", "Reserved" );
	propPtr->addAcceptedValue( "Reserved2", "Reserved" );
	propPtr->addAcceptedValue( "Reserved3", "Reserved" );
	propPtr->addAcceptedValue( "2V6", "Brown-Out at 2.6V (typical)" );
	propPtr->addAcceptedValue( "3V4", "Brown-Out at 3.4V (typical)" );
	propPtr->addAcceptedValue( "3V5", "Brown-Out at 3.5V (typical)" );
	propPtr->addAcceptedValue( "4V3", "Brown-Out at 4.3V (typical)" );
	...
	//Fuse High Byte properties
	propPtr = shared_ptr<Property>(new Property( this, "FUSEHIGH_OCDEN", "Disabled", "Enable On-Chip Debugger", false ));
	propPtr->addAcceptedValue( "Enabled", "OCD enabled" );
	propPtr->addAcceptedValue( "Disabled", "OCD disabled" );
	...
	propPtr = shared_ptr<Property>(new Property( this, "FUSEHIGH_JTAGEN", "Enabled", "Enable JTAG interface", false ));
	...
	propPtr = shared_ptr<Property>(new Property( this, "FUSEHIGH_EESAVE", "Enabled", "EEPROM memory is preserved through the Chip Erase", false ));
	...
	propPtr->addAcceptedValue( "512w",  "Boot size is  512 words (0xFE00-0xFFFF)" );
	propPtr->addAcceptedValue( "1024w",  "Boot size is  1024 words (0xFC00-0xFFFF)" );
	propPtr->addAcceptedValue( "2048w", "Boot size is 2048 words (0xF800-0xFFFF)" );
	propPtr->addAcceptedValue( "4096w", "Boot size is 4096 words (0xF000-0xFFFF)" );
	...
	propPtr->addAcceptedValue( "0001", "Reserved1" );
	propPtr->addAcceptedValue( "0010", "Calibrated Internal RC Oscillator 8.0 MHz" );
	propPtr->addAcceptedValue( "0011", "Reserved2" );
	...
	propPtr->addAcceptedValue( "0110", "Reserved3" );
	propPtr->addAcceptedValue( "0111", "Reserved4" );
	...
	if (std::string("").compare(deviceSpecification) == 0) {
		//Set AT90Usb1297 chip signature string
		this->setSignatureString("1E9782");

		//Set AT90Usb1286 chip name
		this->setChipName("ATMEL AT90Usb1286/1287");
	}
}

unsigned long ChipAT90Usb1286::getExternalClockFrequency() {
...
void ChipAT90Usb1286::setExternalClockFrequency(unsigned long frequency) {
	this->externalClockFrequency = frequency;
}

void ChipAT90Usb1286::setInternalClockFrequency(unsigned long newInternalClockFrequency)  {
	internalClockFrequency = newInternalClockFrequency;
}

unsigned long ChipAT90Usb1286::getInternalClockFrequency() {
	return internalClockFrequency;
}

ChipAT90Usb1286::~ChipAT90Usb1286() {

}

InterfaceClass::ChipIF *ChipAT90Usb1286::createInstance(const std::string &deviceSpecification) {
	return new ChipAT90Usb1286(deviceSpecification);
}

bool ChipAT90Usb1286::compareWithFile(const char* filename) {
	return false;
}

bool ChipAT90Usb1286::compareWithChip(ChipIF& otherChip) {
	return false;
}

string ChipAT90Usb1286::getFactoryNameReference() {
	return "AT90USB1286";
}
