...
namespace ATMega32U4 {
	...
	void updateFuseExtBodLevel(const std::string &value, InterfaceClass::ChipIF& chip) {
		...
		if ( value.compare("2V0") == 0 ) {
			//bits 2,1,0 = 110
			mem.escreve( old_value | 0x06 );
		}
		else if ( value.compare("2V2") == 0 ) {
			//bits 2,1,0 = 101
			mem.escreve( old_value | 0x05 );
		}
		else if ( value.compare("2V4") == 0 ) {
			//bits 2,1,0 = 100
...
ChipATMega32U4::ChipATMega32U4(const std::string &deviceSpecification) {
	using namespace ATMega32U4;
	...
	memPtr =  shared_ptr<Memoria>(new Memoria(16384, 0x0000, 16, "Program"));
	this->addMemory( memPtr->obtem_nome(), memPtr );
	memPtr->fill_value(0xffff, 0x0000, 16384);

	memPtr = shared_ptr<Memoria>(new Memoria(1024, 0x0000, 8, "Data"));
	this->addMemory( memPtr->obtem_nome(), memPtr );
	memPtr->fill_value(0xff, 0x0000, 1024);
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "ProgramPageSizeWordsBits", "6", "The size in bits of each page for the program memory", true ));
	...
	progPropPtr->addAcceptedValue( "256", "The total number of program memory pages is 128" );
	this->addProgrammerProperty( progPropPtr );
	...
	propPtr->addAcceptedValue( "2V0", "Brown-Out at 2.0V (typical)" );
	propPtr->addAcceptedValue( "2V2", "Brown-Out at 2.2V (typical)" );
	propPtr->addAcceptedValue( "2V4", "Brown-Out at 2.4V (typical)" );
	...
	propPtr = shared_ptr<Property>(new Property( this, "FUSEHIGH_BOOTSZ", "2048w", "Select Boot Size", false));
	propPtr->addAcceptedValue( "256w",  "Boot size is  256 words (0x3F00-0x3FFF)" );
	propPtr->addAcceptedValue( "512w",  "Boot size is  512 words (0x3E00-0x3FFF)" );
	propPtr->addAcceptedValue( "1024w", "Boot size is 1024 words (0x3C00-0x3FFF)" );
	propPtr->addAcceptedValue( "2048w", "Boot size is 2048 words (0x3800-0x3FFF)" );
	...
	if (std::string("").compare(deviceSpecification) == 0) {
		//Set ATMega32U4 chip signature string
		this->setSignatureString("1E9587");

		//Set ATMega32U4 chip name
		this->setChipName("ATMEL ATMega32U4");
	}
}

unsigned long ChipATMega32U4::getExternalClockFrequency() {
...
void ChipATMega32U4::setExternalClockFrequency(unsigned long frequency) {
	this->externalClockFrequency = frequency;
}

void ChipATMega32U4::setInternalClockFrequency(unsigned long newInternalClockFrequency)  {
	internalClockFrequency = newInternalClockFrequency;
}

unsigned long ChipATMega32U4::getInternalClockFrequency() {
	return internalClockFrequency;
}

ChipATMega32U4::~ChipATMega32U4() {

}

InterfaceClass::ChipIF *ChipATMega32U4::createInstance(const std::string &deviceSpecification) {
	return new ChipATMega32U4(deviceSpecification);
}

bool ChipATMega32U4::compareWithFile(const char* filename) {
	return false;
}

bool ChipATMega32U4::compareWithChip(ChipIF& otherChip) {
	return false;
}

string ChipATMega32U4::getFactoryNameReference() {
	return "ATMEGA32U4";
}
