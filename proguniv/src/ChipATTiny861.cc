...
ChipATTiny861::ChipATTiny861(const std::string &deviceSpecification) {
	//Add chip memories 
	...
	//Set ATTiny861 chip signature string
	this->setSignatureString("1E930D");
	
	//Set ATTiny861 chip name
	this->setChipName("ATMEL ATTiny861");
}

ChipATTiny861::~ChipATTiny861() {	
}

InterfaceClass::ChipIF *ChipATTiny861::createInstance(const std::string &deviceSpecification) {
	return new ChipATTiny861(deviceSpecification);
}

bool ChipATTiny861::compareWithFile(const char* filename) {
	return false;
}

bool ChipATTiny861::compareWithChip(ChipIF& otherChip) {
	return false;
}

string ChipATTiny861::getFactoryNameReference() {
	return "ATTINY861";
}
