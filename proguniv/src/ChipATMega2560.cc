...
ChipATMega2560::ChipATMega2560(const std::string &deviceSpecification) {
	using namespace ATMega2560;

	//Add chip memories
	shared_ptr<Memoria> memPtr =  shared_ptr<Memoria>(new Memoria(131072, 0x0000, 16, "Program"));
	this->addMemory( memPtr->obtem_nome(), memPtr );
	memPtr->fill_value(0xffff, 0x0000, 131072);
	...
	this->addProgrammerProperty( progPropPtr );
        ...
        progPropPtr = shared_ptr<Property>(new Property( this, "requiresExtendedAddress", "true", "Wether this chip supports and requires the extended address command to address all the flash memory", true ));
	progPropPtr->addAcceptedValue( "true", "This chip supports and requires the LoadExtendedAddress command to adddress the all the flash memory" );
	...
	progPropPtr = shared_ptr<Property>(new Property( this, "ProgramTotalPages", "1024", "The total number of program memory pages in this chip", true ));
	progPropPtr->addAcceptedValue( "1024", "The total number of program memory pages is 1024" );
	this->addProgrammerProperty( progPropPtr );
	...
	//Set ATMega2560 chip signature string
	this->setSignatureString("1E9801");

	//Set ATMega2560 chip name
	this->setChipName("ATMEL ATMega2560");
}

unsigned long ChipATMega2560::getExternalClockFrequency() {
...
void ChipATMega2560::setExternalClockFrequency(unsigned long frequency) {
	this->externalClockFrequency = frequency;
}

void ChipATMega2560::setInternalClockFrequency(unsigned long newInternalClockFrequency)  {
	internalClockFrequency = newInternalClockFrequency;
}

unsigned long ChipATMega2560::getInternalClockFrequency() {
	return internalClockFrequency;
}

ChipATMega2560::~ChipATMega2560() {
}

InterfaceClass::ChipIF *ChipATMega2560::createInstance(const std::string &deviceSpecification) {
	return new ChipATMega2560(deviceSpecification);
}

bool ChipATMega2560::compareWithFile(const char* filename) {
	return false;
}

bool ChipATMega2560::compareWithChip(ChipIF& otherChip) {
	return false;
}

string ChipATMega2560::getFactoryNameReference() {
	return "ATMEGA2560";
}
