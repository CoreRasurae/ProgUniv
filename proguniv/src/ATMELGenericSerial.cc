	...
	for (nbit = 0; nbit < 8; nbit++) {
		...
		if ( generateClockSignal ) {
			for (i = 0; i < cyclesSCK0; i++) {
				...
				if (i == cyclesSCK0 - 1 && receivedByte != 0 ) {
					//Read the bit
...
void ATMELGenericSerial::serialPowerUp() {
	...
	updateExtendedAddressIfRequired(0x00);
}
...
bool ATMELGenericSerial::checkChipSignature(bool *isLocked) {
	unsigned char readback[4];
	...
	serialPowerUpEnterProgramming(true);
		...
		for (i = 0; i < 3; i++) {
			...
			signature[i] = readback[3];
                ...
                if (signature[0] == 0x00 && signature[1] == 0x01 && signature[2] == 0x02) {
                	locked = true;
                }
		...
		result = chip->checkSignature(signatureString);
